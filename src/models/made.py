...
class Made(LightningModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.hparams.conditional:
            x = x[:, self.hparams["conditional_variables_size"] :]
    ...
    def predict_step(
        ...
        if self.hparams.conditional:
            output_size = (
                self.hparams["input_size"] - self.hparams["conditional_variables_size"]
            )
        for spin in range(output_size):
            logits = self.model(batch)
            # generate x_hat according to the compute probability
            batch[
                :, spin + self.hparams["conditional_variables_size"]
            ] = torch.bernoulli(torch.sigmoid(logits[:, spin]))

        # compute the probability of the sample
        if self.hparams.conditional:
            batch = batch[:, self.hparams["conditional_variables_size"] :]
