...
def kmeans(data, num_clusters):
    ...
    Returns:
      ...
      `data`. Two points in `data` i, j are in the same cluster if
      clusters[i] == clusters[j].
    """

    # Select random initial cluster centers.
    centers = np.split(data, num_clusters)
    centers = np.array([center[np.random.randint(len(center))] for center in centers])
...
if __name__ == "__main__":
    cluster_0 = np.random.normal(loc=[0, 0], scale=2.3, size=(3000, 2))
    cluster_1 = np.random.normal(loc=[-3, -1], scale=1.0, size=(3000, 2))
    cluster_2 = np.random.normal(loc=[2, -2], scale=0.5, size=(3000, 2))
